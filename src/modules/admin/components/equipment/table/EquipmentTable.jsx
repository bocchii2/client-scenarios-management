...
const EquipmentTable = ({ equipment = [], loading = false }) => {
  ...
  }, [searchTerm, equipment, loading]);

  React.useEffect(() => {
    setSearchResultsCount(filteredEquipment.length);
  }, [filteredEquipment]);
  ...
  const handleView = (equipmentId) => {
    console.log("Ver equipamiento con ID:", equipmentId);
  };

  const handleEdit = (equipmentId) => {
    console.log("Editar equipamiento con ID:", equipmentId);
  };

  const handleDelete = (equipmentId) => {
    console.log("Eliminar equipamiento con ID:", equipmentId);
  };
  ...
  return (
    <React.Fragment>
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 mb-4 sm:mb-6">
        ...
        <div className="mt-4 pt-4 border-t border-gray-200">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <span>
              Mostrando {searchResultsCount} equipamiento{searchResultsCount !== 1 ? 's' : ''}
              {(searchTerm) && !loading && (
      ...
      <Table>
        <Thead headers={equipmentHeader} hasExpand={false} />
        <Tbody>
          {loading ? (
            ...
            <tr className="hover:bg-gray-200">
              <td
                colSpan={equipmentHeader.length}
                className="px-6 py-10 text-center bg-white"
              >
                <div className="mx-auto max-w-md flex flex-col items-center gap-4 text-gray-500">
                  <p className="font-light italic">No hay equipamiento disponible.</p>
                  <span>
                    Para poder agregar equipamiento, haz clic en el siguiente botón:
                  </span>
                  <div className="w-full sm:w-auto">
                    <Button
                      ...
                      icon={<MdAdd />}
                      size="medium"
                      ariaLabel="Agregar un nuevo equipamiento"
                      label="Agregar un nuevo equipamiento"
                    />
                  </div>
                </div>
              </td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  <div className="flex justify-center gap-2">
                    <Button
                      ...
                      ariaLabel={`Ver equipamiento ${item.name}`}
                      disabled={loading}
                      loading={false}
                    />
                    <Button
                      ...
                      ariaLabel={`Editar equipamiento ${item.name}`}
                      disabled={loading}
                      loading={false}
                    />
                    <Button
                      ...
                      ariaLabel={`Eliminar equipamiento ${item.name}`}
                      disabled={loading}
                      loading={false}
                    />
                  </div>
